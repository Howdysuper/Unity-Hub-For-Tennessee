...
export default function ResourceAssistant() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (!apiKey) {
        console.error('GEMINI_API_KEY is missing. Please ensure it is set in your environment variables.');
        setMessages(prev => [...prev, { role: 'bot', text: "I'm sorry, the AI Assistant is not configured correctly. Please check the environment variables (GEMINI_API_KEY)." }]);
        setIsLoading(false);
        return;
      }
      ...
      let errorMessage = "I'm having a bit of trouble connecting right now. Please try again in a moment.";
      
      if (error?.message?.includes('429') || error?.message?.toLowerCase().includes('quota')) {
        errorMessage = "The AI is currently receiving too many requests (Free Tier limit). Please wait about 60 seconds and try again.";
      }
